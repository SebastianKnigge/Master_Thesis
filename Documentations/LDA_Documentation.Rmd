...
The function *set_up_books()* returns a warning that several books seem to consist of only one chapter. In order to get a corpus consisting out of $6$ books, the function *append_by_chaper()* is used, which fills up the corpus to the desired number of books.
...
We also want to compare this to a reduction of the word dictionary by the TFIDF. We are using a reduction by 50\% of the dimension of the original bag of words.

```{r chapters_dtm_tfidf, chache=FALSE, warning=FALSE}
chapters_dtm_tfidf <- convert_to_dtm_2(word_counts, top=(0.1*M))
ncol(chapters_dtm_tfidf)
```


## Application of LDA Model to Full Corpus

### Fitting via VEM

We set up the LDA model for the shrinked embedding corpus via frequency=2. In a first try we are using the default "VEM-Algorithm" to fit the model.
...
The third LDA fit builds up on data from the shrinked dictionary/bag of words by TFIDF.

```{r chapters_lda_tfidf, cache=FALSE, warning=FALSE, message=FALSE}
tim1 <- Sys.time()
...
Now we exclude for each of the 3 embedings the - so called - beta matrix and compare the most likely result with the real results. Depending on how good the LDA will seperate the books, this influences the goodness of the fit.
...
### Comparison to Fit via Gibbs Sampling
...
We now can evaluate several fits of a model for different splits. Here is a parallelization for the individual for loops applied. In this case only embeding via TFIDF is used.
...
cl <- parallel::makeCluster(useable_cores)
doParallel::registerDoParallel(cl)
embedding_performance_matrix_sec <- foreach(i = 1:length(fractions), .combine = 'rbind', .export = ls(.GlobalEnv), .packages = c("dplyr", "udpipe", "topicmodels", "tidyr", "tidytext")) %dopar% {
...
First we will sample the books, the same way as is it was done in Example 1.
...
cl <- parallel::makeCluster(useable_cores)
doParallel::registerDoParallel(cl)
embedding_performance_matrix_10 <- foreach(i = 1:length(fractions), .combine = 'rbind', .export = ls(.GlobalEnv), .packages = c("dplyr", "udpipe", "topicmodels", "tidyr", "tidytext")) %dopar% {
...
In the first place, we try the in-model-validation procedure, for each of the 3 embeddings. Table \ref{in-model_val} displays the results of each validation.
...
rownames(performance_matrix) <- c("missc. rate", "time")
performance_matrix %>%  stargazer(summary=FALSE, header=F, title="In-Model-Validation for the 3 embeddings", label = "in-model_val")
```
