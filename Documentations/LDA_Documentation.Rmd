...
# Applicate the LDA model on the full corpus
...
Surprisingly, the run-time to fit the LDA model for the embedding using all words, does not take way longer than for the other two embeddings.
...
The fit_n_evaluate finction will fit the LDA model and evaluate the goodness of fit for an object of the function split_for_fit.
...
We now can evaluate severat fits of a model for different splits.
