...
A second possibility is to extract the tfidfs of the whole topics. First you have to map the topics to the documents within the tidytext format. Only then can the tfidf_tf matrix be set up for the individual topics.
...
Now there are two methods to create the word clouds for the tfidf measure. once by aggregating the individual tfidfs of the documents, as it is done in the following. 

```{r wordcloud_tfidf, message=FALSE, warning=FALSE}
par(mfrow=c(2,4))
par(mar=c(1,1,1,1))
set.seed(123)
plot_wordcloud(corpus, selection=ind1[,1], i=1, scale=c(2.1,0.05), 
               max.words = 45)
...
plot_wordcloud(corpus, selection=ind6[,1], i=6, scale=c(1.8,0.001), 
               max.words = 40)
plot_wordcloud(corpus, selection=ind7[,1], i=7, scale=c(2,0.2), 
               max.words = 45) 
```
...
plot_wordcloud_topic(corpus, topic_select=3, scale=c(1.6,0.005), 
               max.words = 40)
plot_wordcloud_topic(corpus, topic_select=4, max.words = 45, scale=c(2.2, 0.3))
...
Now it's interesting to see if embedding with tfidf will cluster other groups or the same. So we will reduce the Document Term Matrix to $0.8*\text{M}$ words, which amounts to a reduction by 20\%

```{r, cache=TRUE}
tim1 <- Sys.time()
dtm_50 <- dtm %>% dtm_remove_tfidf(top=0.8*M)
set.seed(123)
