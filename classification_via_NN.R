...
barplot(table(adjusted_format$topics))

sample1 <- adjusted_format$y %>% balancedcluster(m=0.8, cluster=adjusted_format$topics)
adjusted_format$topics[sample1[,1]==1]


# sampling does not provide an appropriate function to sample for my
# usecase
testing_ratio <- 0.1
cluster=adjusted_format$topics
X=adjusted_format$y
unique(cluster)
freq_per_cluster <- (table(cluster)*testing_ratio) %>% floor() %>% 
